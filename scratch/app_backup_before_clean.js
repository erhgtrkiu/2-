...
// Звуковые файлы MP3
const sndSteps = new Audio('sounds/steps.mp3');
const sndRun = new Audio('sounds/run.mp3');
const sndBag = new Audio('sounds/bag.mp3');
const sndShot = new Audio('sounds/shot.mp3');
const sndDoorOpen = new Audio('sounds/door (open).mp3');
const sndDoorClose = new Audio('sounds/door (closing).mp3');
...
function unlockAchievement(id) {
    ...
    logToConsole(t("log_ach_unlocked", ach.title), "loot");
}
...
function updateAchievementsModalUI() {
    ...
    for (let id in ACHIEVEMENTS) {
        ...
        const displayTitle = isSecretLocked ? t("ach_secret_title") : ach.title;
        const displayDesc = isSecretLocked ? t("ach_secret_hint") : ach.description;
        ...
        card.innerHTML = `
            ...
            <div class="achievement-card-status">
                ${ach.unlocked ? t('ach_status_unlocked') : t('ach_status_locked')}
            </div>
        `;
...
let playerYaw = 0;
let mouseSensMultiplier = parseFloat(localStorage.getItem('samosbor_sensitivity')) || 1.0; // Вращение по горизонтали
...
function init3D() {
    ...
    camera = new THREE.PerspectiveCamera(65, holder.clientWidth / holder.clientHeight, 0.02, 100);
    camera.rotation.order = 'YXZ'; // Важно для FPS вращения!
    
    shootFlashLight = new THREE.PointLight(0xfff0c0, 0, 30);
    scene.add(shootFlashLight);
...
// --- Load 3D Pistol Model ---
if (typeof THREE.GLTFLoader !== 'undefined') {
    const loader = new THREE.GLTFLoader();
    loader.load('usp/source/USPS.glb', (gltf) => {
        ...
        // 4. FORCE REPLACE all materials with bright visible material
...
// --- End pistol model --- — using full 3D model instead
...
function build3DScene() {
    ...
    for (let i = 1; i >= -1; i--) {
        ...
        if (floorNum > START_FLOOR) continue; // Skip generating non-existent floors above START_FLOOR
        const baseY = i * 5.0;
        const isTopMost = (floorNum === START_FLOOR) || (i === 1 && state.floor + 1 === START_FLOOR) || (i === 1);
        buildFloor(floorNum, baseY, isTopMost);
    }
...
function addVolumetricHandle(doorMesh) {
    ...
    const shaftGeo = new THREE.CylinderGeometry(0.036, 0.036, 0.06, 8);
    shaftGeo.rotateX(Math.PI / 2); // Align pointing along Z-axis
    
    const wheelGeo = new THREE.TorusGeometry(0.24, 0.024, 8, 24);
    
    const spokeHGeo = new THREE.CylinderGeometry(0.016, 0.016, 0.48, 8);
    spokeHGeo.rotateZ(Math.PI / 2); // Align pointing along X-axis
    
    const spokeVGeo = new THREE.CylinderGeometry(0.016, 0.016, 0.48, 8); // Align pointing along Y-axis

    [-1, 1].forEach((side) => {
        const shaftZ = side * (0.05 + 0.03);
        const wheelZ = side * (0.05 + 0.06 + 0.012);

        // 1. Shaft
        const shaft = new THREE.Mesh(shaftGeo, handleMat);
        shaft.position.set(0.0, 0, shaftZ);
        ...
        wheel.position.set(0.0, 0, wheelZ);
        ...
        spokeH.position.set(0.0, 0, wheelZ);
        ...
        spokeV.position.set(0.0, 0, wheelZ);
...
function buildFloor(floorNum, baseY, isTopMost = false) {
    ...
    if (isTopMost) {
        // Добавляем стены лестницы на этаж выше, чтобы закрыть пустоту (void)
        const topShaftL = new THREE.Mesh(new THREE.BoxGeometry(0.2, 5.0, 7.5), wallMat);
        topShaftL.position.set(-1.6, baseY + 2.5, -45.75);
        scene.add(topShaftL);
        staircaseMeshes.push(topShaftL);
        
        const topShaftR = new THREE.Mesh(new THREE.BoxGeometry(0.2, 5.0, 7.5), wallMat);
        topShaftR.position.set(1.6, baseY + 2.5, -45.75);
        scene.add(topShaftR);
        staircaseMeshes.push(topShaftR);
        
        const topShaftB = new THREE.Mesh(new THREE.BoxGeometry(3.2, 5.0, 0.2), wallMat);
        topShaftB.position.set(0, baseY + 2.5, -49.6);
        scene.add(topShaftB);
        staircaseMeshes.push(topShaftB);
        
        // Крыша лестничного колодца
        const topShaftCeil = new THREE.Mesh(new THREE.BoxGeometry(3.4, 0.2, 7.7), ceilingMat);
        topShaftCeil.position.set(0, baseY + 5.0 + 0.1, -45.75);
        scene.add(topShaftCeil);
        staircaseMeshes.push(topShaftCeil);
    }
    ...
    DOOR_LAYOUT.forEach((layout, idx) => {
        ...
        if (doorObj) {
            ...
            if (doorObj.type === 'apartment' || doorObj.type === 'monster') {
                ...
                const isTechRoom = ['warehouse', 'archive', 'compressor', 'control', 'ventilation'].includes(doorObj.roomType);
                const currentFloorMat = isTechRoom ? transWallMat : roomFloorMat;
                const currentWallMat = isTechRoom ? transWallMat : roomWallMat;
                const currentCeilMat = isTechRoom ? transWallMat : roomCeilMat;
                
                const rFloor = new THREE.Mesh(new THREE.BoxGeometry(7.0, 0.1, 7.0), currentFloorMat);
                rFloor.position.set(roomCenterX, baseY - 0.05, roomCenterZ);
                rFloor.receiveShadow = true;
                roomGroup.add(rFloor);
                
                const rCeil = new THREE.Mesh(new THREE.BoxGeometry(7.0, 0.1, 7.0), currentCeilMat);
                rCeil.position.set(roomCenterX, baseY + 5.0 + 0.05, roomCenterZ);
                rCeil.receiveShadow = true;
                roomGroup.add(rCeil);
                
                const rBackWall = new THREE.Mesh(new THREE.BoxGeometry(0.2, 5.0, 7.0), currentWallMat);
                ...
                const rSideWall1 = new THREE.Mesh(new THREE.BoxGeometry(7.0, 5.0, 0.2), currentWallMat);
                ...
                const rSideWall2 = new THREE.Mesh(new THREE.BoxGeometry(7.0, 5.0, 0.2), currentWallMat);
                ...
                const rFrontWall1 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 5.0, 2.9), currentWallMat);
                ...
                const rFrontWall2 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 5.0, 2.9), currentWallMat);
                ...
                const rFrontWallTop = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.4, 1.2), currentWallMat);
                ...
                if (Math.abs(floorNum - state.floor) <= 1) {
                    const roomLightColor = isTechRoom ? 0xccffbb : (doorObj.roomType === 'contaminated' ? 0x99ff99 : 0xffeedd);
                    const roomLight = new THREE.PointLight(roomLightColor, doorObj.opened ? 0.35 : 0, 9);
                ...
                if (doorObj.roomType === 'bedroom_kitchen') {
                    // Partition walls (dividing bedroom and kitchen along Z-axis)
                    const rPartWall1 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 5.0, 2.9), currentWallMat);
                    rPartWall1.position.set(roomCenterX, baseY + 2.5, roomCenterZ - 2.05);
                    rPartWall1.castShadow = true;
                    rPartWall1.receiveShadow = true;
                    roomGroup.add(rPartWall1);
                    
                    const rPartWall2 = new THREE.Mesh(new THREE.BoxGeometry(0.2, 5.0, 2.9), currentWallMat);
                    rPartWall2.position.set(roomCenterX, baseY + 2.5, roomCenterZ + 2.05);
                    rPartWall2.castShadow = true;
                    rPartWall2.receiveShadow = true;
                    roomGroup.add(rPartWall2);
                    
                    const rPartWallTop = new THREE.Mesh(new THREE.BoxGeometry(0.2, 2.4, 1.2), currentWallMat);
                    rPartWallTop.position.set(roomCenterX, baseY + 3.8, roomCenterZ);
                    rPartWallTop.castShadow = true;
                    rPartWallTop.receiveShadow = true;
                    roomGroup.add(rPartWallTop);
                    
                    // Inner hermetic door pivot and mesh (swinging around Z = roomCenterZ - 0.6)
                    const innerDoorPivot = new THREE.Group();
                    innerDoorPivot.name = `innerDoorPivot_${floorNum}_${idx}`;
                    innerDoorPivot.position.set(roomCenterX, baseY + 1.3, roomCenterZ - 0.6);
                    
                    const innerDoorMesh = new THREE.Mesh(new THREE.BoxGeometry(1.2, 2.6, 0.1), doorMat);
                    innerDoorMesh.rotation.y = Math.PI / 2;
                    innerDoorMesh.position.set(0, 0, 0.6);
                    innerDoorMesh.castShadow = true;
                    innerDoorMesh.receiveShadow = true;
                    innerDoorPivot.add(innerDoorMesh);
                    
                    // Add volumetric handle (wheel)
                    addVolumetricHandle(innerDoorMesh);
                    
                    if (doorObj.innerOpened) {
                        innerDoorPivot.rotation.y = Math.PI / 2;
                    }
                    
                    roomGroup.add(innerDoorPivot);
                    
                    // Bedroom furniture (Back Room: X > roomCenterX)
                    // 1. Bed Frame
                    const bedFrameMat = new THREE.MeshStandardMaterial({ color: 0x4d3319, roughness: 0.8 });
                    const bedFrame = new THREE.Mesh(new THREE.BoxGeometry(1.6, 0.4, 2.2), bedFrameMat);
                    bedFrame.position.set(layout.x + 5.5 * dirX, baseY + 0.2, roomCenterZ - 1.8);
                    roomGroup.add(bedFrame);
                    // 2. Mattress
                    const mattressMat = new THREE.MeshStandardMaterial({ color: 0xdddddd, roughness: 0.9 });
                    const mattress = new THREE.Mesh(new THREE.BoxGeometry(1.5, 0.25, 2.1), mattressMat);
                    mattress.position.set(layout.x + 5.5 * dirX, baseY + 0.5, roomCenterZ - 1.8);
                    roomGroup.add(mattress);
                    // 3. Pillow
                    const pillow = new THREE.Mesh(new THREE.BoxGeometry(1.3, 0.1, 0.4), mattressMat);
                    pillow.position.set(layout.x + 5.5 * dirX, baseY + 0.65, roomCenterZ - 2.6);
                    roomGroup.add(pillow);
                    
                    // Cabinet
                    const cab = createDetailedCabinet(dirX);
                    cab.position.set(layout.x + 5.5 * dirX, baseY, roomCenterZ + 2.2);
                    roomGroup.add(cab);
                    
                    // Kitchen furniture (Front Room: X < roomCenterX)
                    // 1. Kitchen counter
                    const counterMat = new THREE.MeshStandardMaterial({ color: 0x222222, roughness: 0.5 });
                    const counter = new THREE.Mesh(new THREE.BoxGeometry(1.8, 0.9, 0.7), counterMat);
                    counter.position.set(layout.x + 2.0 * dirX, baseY + 0.45, roomCenterZ + 2.4);
                    roomGroup.add(counter);
                    // 2. Table
                    const table = createDetailedTable();
                    table.position.set(layout.x + 2.0 * dirX, baseY, roomCenterZ - 1.8);
                    roomGroup.add(table);
                    
                } else if (isTechRoom) {
                    // Industrial/technical room furniture (crates, panels)
                    // 1. Detailed Panel
                    const panel = createDetailedPanel(dirX);
                    panel.position.set(roomCenterX + 1.5 * dirX, baseY, roomCenterZ + 1.5);
                    roomGroup.add(panel);
                    
                    // 2. Metallic boxes/crates
                    const crateMat = new THREE.MeshStandardMaterial({ color: 0x555555, roughness: 0.6, metalness: 0.7 });
                    const crate1 = new THREE.Mesh(new THREE.BoxGeometry(1.0, 1.0, 1.0), crateMat);
                    crate1.position.set(roomCenterX - 1.5 * dirX, baseY + 0.5, roomCenterZ - 1.5);
                    roomGroup.add(crate1);
                    
                    const crate2 = new THREE.Mesh(new THREE.BoxGeometry(0.8, 0.8, 0.8), crateMat);
                    crate2.position.set(roomCenterX - 1.5 * dirX, baseY + 0.4, roomCenterZ + 1.5);
                    roomGroup.add(crate2);
                    
                } else {
                    // Standard single room apartment layout
                    const cab = createDetailedCabinet(dirX);
        ...
        if (nextZ < -42) {
            ...
            if (allowed) {
                ...
                if (playerPos.y < -4.7) {
                    ...
                    if (Math.random() < 0.1 && !state.stairsMonsterActive) {
                        ...
                        logToConsole(t("log_descend_floor", state.floor), "sys");
                    }
            ...
            DOOR_LAYOUT.forEach((layout, idx) => {
                ...
                if (doorObj.type !== 'empty') {
                    const isRoom = doorObj.type === 'apartment' || doorObj.type === 'monster';
                    const roomWidth = isRoom ? 3.5 : 2.5;
                    const roomDepth = isRoom ? 7.0 : 5.0;
                    const dirX = isLeft ? -1 : 1;
                    ...
                    // Furniture and partition colliders inside rooms (only if safe door, since player cannot enter monster rooms)
                    if (doorObj.type === 'apartment') {
                        const roomCenterX = layout.x + 3.7 * dirX;
                        const isTechRoom = ['warehouse', 'archive', 'compressor', 'control', 'ventilation'].includes(doorObj.roomType);
                        
                        if (doorObj.roomType === 'bedroom_kitchen') {
                            // Partition wall segments (Z-aligned at roomCenterX)
                            colliders.push({ minX: roomCenterX - 0.1, maxX: roomCenterX + 0.1, minZ: layout.z - 3.5, maxZ: layout.z - 0.6 });
                            colliders.push({ minX: roomCenterX - 0.1, maxX: roomCenterX + 0.1, minZ: layout.z + 0.6, maxZ: layout.z + 3.5 });
                            
                            // If inner door is closed, block the doorway segment
                            if (!doorObj.innerOpened) {
                                colliders.push({ minX: roomCenterX - 0.1, maxX: roomCenterX + 0.1, minZ: layout.z - 0.6, maxZ: layout.z + 0.6 });
                            }
                            
                            // Cabinet in bedroom (back room)
                            const cabX = layout.x + 5.5 * dirX;
                            const cabZ = layout.z + 2.2;
                            colliders.push({ minX: Math.min(cabX - 1.0, cabX + 1.0), maxX: Math.max(cabX - 1.0, cabX + 1.0), minZ: cabZ - 0.5, maxZ: cabZ + 0.5 });
                            
                            // Bed in bedroom (back room)
                            const bedX = layout.x + 5.5 * dirX;
                            const bedZ = layout.z - 1.8;
                            colliders.push({ minX: Math.min(bedX - 0.85, bedX + 0.85), maxX: Math.max(bedX - 0.85, bedX + 0.85), minZ: bedZ - 1.25, maxZ: bedZ + 1.25 });
                            
                            // Kitchen counter (front room)
                            const counterX = layout.x + 2.0 * dirX;
                            const counterZ = layout.z + 2.4;
                            colliders.push({ minX: Math.min(counterX - 1.0, counterX + 1.0), maxX: Math.max(counterX - 1.0, counterX + 1.0), minZ: counterZ - 0.45, maxZ: counterZ + 0.45 });
                            
                            // Kitchen table (front room)
                            const tableX = layout.x + 2.0 * dirX;
                            const tableZ = layout.z - 1.8;
                            colliders.push({ minX: Math.min(tableX - 0.9, tableX + 0.9), maxX: Math.max(tableX - 0.9, tableX + 0.9), minZ: tableZ - 0.6, maxZ: tableZ + 0.6 });
                            
                        } else if (isTechRoom) {
                            // Panel
                            const panelX = roomCenterX + 1.5 * dirX;
                            const panelZ = layout.z + 1.5;
                            colliders.push({ minX: panelX - 0.4, maxX: panelX + 0.4, minZ: panelZ - 0.8, maxZ: panelZ + 0.8 });
                            
                            // Crate 1
                            const crate1X = roomCenterX - 1.5 * dirX;
                            const crate1Z = layout.z - 1.5;
                            colliders.push({ minX: crate1X - 0.6, maxX: crate1X + 0.6, minZ: crate1Z - 0.6, maxZ: crate1Z + 0.6 });
                            
                            // Crate 2
                            const crate2X = roomCenterX - 1.5 * dirX;
                            const crate2Z = layout.z + 1.5;
                            colliders.push({ minX: crate2X - 0.5, maxX: crate2X + 0.5, minZ: crate2Z - 0.5, maxZ: crate2Z + 0.5 });
...
function performInteractionRaycast(force = false) {
    ...
    if (intersects.length > 0) {
        ...
        // 5. Spawn Door
        else if (hitObj.userData.type === 'spawnDoor' && dist < 4.0) {
            hitSpawnDoor = true;
        }
    }
    ...
    state.focusedSpawnDoor = hitSpawnDoor;
...
function updateHackSliderTuner() {
    ...
    if (diff < 1.5) {
        strengthLabel.innerText = t("hack_sig_aligned");
        strengthLabel.style.color = '#00ff66';
    } else if (diff < 5.0) {
        strengthLabel.innerText = t("hack_sig_weak");
        strengthLabel.style.color = '#ffcc00';
    } else if (diff < 12.0) {
        strengthLabel.innerText = t("hack_sig_interference");
        strengthLabel.style.color = '#ffaa00';
    } else {
        strengthLabel.innerText = t("hack_sig_mismatch");
...
function submitHackTuning() {
    ...
    if (diff <= 1.5) {
        ...
        logToConsole(t('log_hack_bypass_ok', activeHackChannel), 'loot');
        ...
        logToConsole(t("log_hack_bypass_err"), "danger");
    }
}
...
function useHackerTool() {
    ...
    if (state.focusedSpawnDoor || (Math.abs(playerPos.z) <= 2.5 && !state.focusedStairsDoor)) {
        if (!state.hasHackerTool) {
            logToConsole(t("hacker_warn"), "warn");
            return;
        }
        ...
        if (state.spawnDoorHackAttempts === 1) {
            logToConsole(t("log_sis_warn_1"), "warn");
        } else if (state.spawnDoorHackAttempts === 2) {
            logToConsole(t("log_sis_warn_2"), "warn");
        } else if (state.spawnDoorHackAttempts === 3) {
            logToConsole(t("log_sis_warn_3"), "warn");
            ...
            logToConsole(t("log_samosbor_warn_1"), "danger");
            logToConsole(t("log_samosbor_warn_2"), "danger");
    ...
    // Last Floor check
    if (state.floor === 1) {
        logToConsole(t("log_hacker_final_floor_error"), "danger");
        ...
        setTimeout(() => {
            ...
            if (state.notesCount === 10 && allNotesRead) {
                ...
                if (state.notesCount === 10 && !allNotesRead) {
                    logToConsole(t("log_hack_note_samosbor"), "danger");
                }
                triggerGameOver("ending_2");
            }
        }, 3000);
    ...
    if (!state.hasHackerTool) {
        logToConsole(t("log_no_hacker_tool"), "warn");
        return;
    }
    
    // Check battery
    if (!window._infBatteries && state.batteries <= 0) {
        logToConsole(t("log_hacker_battery_empty"), "warn");
        return;
    }
    ...
    startHackPuzzle(() => {
        ...
        logToConsole(t('log_hack_gate', sdState.opened ? t('log_hack_gate_open') : t('log_hack_gate_close')), 'action');
    ...
    const progressInterval = setInterval(() => {
        ...
        if (progress >= 100) {
            ...
            if (door.roomType === 'nest' && Math.random() < 0.50) {
                ...
                // Show loot selection UI (reuses room-overlay)
...
function showLootUI(items, doorIdx, containerKey) {
    ...
    html += '<button onclick="takeAllLoot()" style="flex:1;padding:8px;background:rgba(255,204,0,0.1);border:1px solid #ffcc00;color:#ffcc00;cursor:pointer;font-family:inherit;font-weight:bold;border-radius:3px;">' + t('loot_take_all') + '</button>';
...
function applyLootItem(item) {
    switch (item.type) {
        case 'ammo':
            ...
            logToConsole(t('loot_item_ammo', item.count || 6), 'loot');
            break;
        case 'bandage':
            state.bandages = (state.bandages || 0) + (item.count || 1);
            logToConsole(t('loot_item_bandage') + ' (+' + (item.count || 1) + ')', 'loot');
            break;
        case 'water':
            state.bottleWater = Math.min(100, state.bottleWater + (item.count || 30));
            logToConsole(t('loot_item_water', item.count || 30), 'loot');
            break;
        case 'battery':
            state.batteries = (state.batteries || 0) + (item.count || 1);
            logToConsole(t('loot_item_battery') + ' (+1)', 'loot');
            break;
        case 'filter':
            state.filter = Math.min(MAX_FILTER, state.filter + (item.count || 30));
            logToConsole(t('loot_item_filter', item.count || 30), 'loot');
            break;
        case 'note':
            if (state.notesCount < LORE_NOTES.length) {
                ...
                logToConsole(t('log_loot_note', LORE_NOTES[nid].title), 'loot');
                openNotesModal(nid);
            }
            break;
        case 'hacker_tool':
            if (!state.hasHackerTool) {
                state.hasHackerTool = true;
                logToConsole(t('log_loot_hacker'), 'loot');
                updateInventoryUI();
            }
            break;
    }
...
function lockRoom() {
    ...
    if (distToOuterDoorX > 2.0 || distToOuterDoorZ > 2.0) {
        logToConsole(t("log_approach_door_to_lock"), "warn");
        return;
    }
    ...
    playSoundDoor(state.samosborSafe);
    
    if (state.samosborSafe) {
        logToConsole(t("log_door_tightened"), "action");
    } else {
        logToConsole(t("log_door_untightened"), "warn");
    }
...
function exitRoom() {
    ...
    playSoundDoor(false);
    logToConsole(t("log_exited_to_hallway"), "action");
...
function triggerFloorDescent() {
    ...
    // 1. Day/Night Cycle calculations
    const cycleDuration = 120000; // 120 seconds
    const progress = (time % cycleDuration) / cycleDuration;
    childSpawnTimer = 25000 + Math.random() * 30000; // 25-55 seconds
...
function getOrGenerateFloorDoors(floorNum) {
    ...
    if (hasMonster) {
        ...
        for (let s of slots) {
            if (doors[s].type === 'empty' || doors[s].type === 'apartment') {
    ...
    doors.forEach((door, idx) => {
        if (door.type === 'apartment' || door.type === 'monster') {
            ...
            if (rTypeRand < 0.30) {
                rType = 'bedroom_kitchen';
            } else if (rTypeRand < 0.40) {
                rType = 'armory';
                rNum = Math.floor(Math.random() * 90 + 10);
            } else if (rTypeRand < 0.50) {
                rType = 'contaminated';
            } else if (rTypeRand < 0.60) {
                rType = 'nest';
                rNum = Math.floor(Math.random() * 90 + 10);
            } else if (rTypeRand < 0.70) {
                rType = 'warehouse';
            } else if (rTypeRand < 0.78) {
                rType = 'archive';
            } else if (rTypeRand < 0.86) {
                rType = 'compressor';
            } else if (rTypeRand < 0.93) {
                rType = 'control';
            } else {
                rType = 'ventilation';
            }
            ...
            door.roomType = rType;
            door.number = rNum;
            door.name = getLocalizedDoorName(door);
...
function enterTransition() {
    ...
    playSoundDoor(true);
    logToConsole(t("log_gate_closed_descending"), "sys");
    ...
    setTimeout(() => {
        logToConsole(t("log_descent_complete", state.floor), "sys");
...
function toggleGasMask() {
    ...
    playSoundMask();
    if (state.maskOn) {
        logToConsole(t("log_gasmask_equip"), "action");
        if (audioCtx) {
            const now = audioCtx.currentTime;
            const osc = audioCtx.createOscillator();
            const gain = audioCtx.createGain();
        ...
        logToConsole(t("log_gasmask_unequip"), "action");
    }
    updateHUD();
}
...
function drinkWater() {
    ...
    if (state.water >= MAX_WATER) {
        logToConsole(t("log_drink_full"), "warn");
        return;
    }
    
    if (state.bottleWater <= 0) {
        logToConsole(t("log_bottle_empty"), "warn");
        return;
    }
    ...
    if (actualTaken <= 0) {
        logToConsole(t("log_drink_quenched"), "warn");
        return;
    }
    ...
    playSoundDrink();
    logToConsole(t("log_drink_sip", Math.round(state.bottleWater)), "action");
...
function useBandage() {
    ...
    if ((state.bandages || 0) <= 0) {
        logToConsole(t("log_no_bandages"), "warn");
        return;
    }
    if (state.health >= MAX_HEALTH) {
        logToConsole(t("log_healthy"), "warn");
        return;
    }
    ...
    logToConsole(t("log_bandage_used"), "action");
    updateHUD();
}
...
function triggerStairsMonster() {
    ...
    playSoundMonsterHiss();
    logToConsole(t("log_stairs_monster_threat"), "danger");
    logToConsole(t("log_stairs_monster_shoot_time"), "danger");
...
function performPlayerKick() {
    ...
    if (hallwayCrawlerMesh && state.location === 'hallway') {
        ...
        if (dist < 3.5) {
            ...
            crawlerHealth -= 1;
            playKickHitSound();
            logToConsole(t("log_kick_hit"), "warn");
            
            if (crawlerHealth <= 0) {
                logToConsole(t("log_kick_kill"), "loot");
                scene.remove(hallwayCrawlerMesh);
                hallwayCrawlerMesh = null;
    ...
    logToConsole(t("log_kick_air"), "sys");
}

function shootPistol() {
    initAudio();
    if (state.ammo <= 0) {
        logToConsole(t("log_no_ammo"), "danger");
        return;
    }
    ...
    // Вспышка в Three.js
    if (scene && shootFlashLight) {
        shootFlashLight.intensity = 4.0;
        shootFlashLight.position.set(camera.position.x, camera.position.y, camera.position.z - 1.0);
        setTimeout(() => { if (shootFlashLight) shootFlashLight.intensity = 0; }, 60);
    }
    
    logToConsole(t("log_shoot"), "action");
    ...
    if (hallwayCrawlerMesh) {
        ...
        if (intersects.length > 0) {
            crawlerHealth -= 1;
            if (crawlerHealth <= 0) {
                logToConsole(t("log_shoot_kill"), "loot");
                scene.remove(hallwayCrawlerMesh);
                hallwayCrawlerMesh = null;
                // Progress gigahrush butcher
                progressAchievement('gigahrush_butcher', 1);
            } else {
                logToConsole(t("log_shoot_hit"), "warn");
                playScaryScreechSound();
            }
            updateHUD();
            return;
    ...
    if (state.stairsMonsterActive) {
        ...
        logToConsole(t("log_stairs_monster_scared"), "sys");
        ...
        logToConsole(t("log_shoot_empty"), "warn");
    }
...
function enableMenuWakeupTheme() {
    ...
    if (!menuNatureCanvas) {
        ...
        menuNatureCanvas.style.opacity = '0.85';
        menuNatureCanvas.style.filter = 'blur(1px)';
...
function animateMenuNature() {
    ...
    for (let i = 0; i < 25; i++) {
        particles.push({
            x: Math.random() * 800,
            y: Math.random() * 600,
            vx: 0.03 + Math.random() * 0.12, // Slower horizontal drift
            vy: -0.03 - Math.random() * 0.06, // Slower upward movement
            size: 1.5 + Math.random() * 3,
            alpha: 0.1 + Math.random() * 0.4,
            wiggleSpeed: 0.0006 + Math.random() * 0.0012, // Slower wiggle
    ...
    // Trees array (growth and sway speeds reduced by ~3-4x)
    const trees = [
        { x: 0.78, targetScale: 1.0, currentScale: 0.05, growthSpeed: 0.000008, trunkColor: '#5c4033', leavesColor1: 'rgba(56, 176, 0, 0.85)', leavesColor2: 'rgba(0, 114, 0, 0.85)', swaySpeed: 0.00025, swayAmp: 14 },
        { x: 0.60, targetScale: 0.75, currentScale: 0.05, growthSpeed: 0.00001, trunkColor: '#4d3319', leavesColor1: 'rgba(76, 196, 20, 0.85)', leavesColor2: 'rgba(20, 134, 10, 0.85)', swaySpeed: 0.00035, swayAmp: 10 },
        { x: 0.18, targetScale: 0.85, currentScale: 0.05, growthSpeed: 0.000007, trunkColor: '#6e473b', leavesColor1: 'rgba(96, 206, 40, 0.85)', leavesColor2: 'rgba(40, 154, 30, 0.85)', swaySpeed: 0.0002, swayAmp: 12 }
    ];

    // Children array
    let childrenSilhouettes = [];
    ...
    function loop() {
        ...
        // 1. Day/Night Cycle calculations (increased from 35s to 105s for a calmer vibe)
        const cycleDuration = 105000;
        const progress = (time % cycleDuration) / cycleDuration;
        ...
        // Key colors
        const nightTop = '#050c1e', nightBottom = '#0e1726';
        const sunriseTop = '#ff7e5f', sunriseBottom = '#feb47b';
        const dayTop = '#7ec0ee', dayBottom = '#e0f6ff';
        const sunsetTop = '#fe8c00', sunsetBottom = '#f83600';
        
        if (progress < 0.2) { // Sunrise
            const f = progress / 0.2;
            skyTop = lerpColor(nightTop, sunriseTop, f);
            skyBottom = lerpColor(nightBottom, sunriseBottom, f);
            ambientLight = 0.3 + 0.4 * f;
        } else if (progress < 0.5) { // Day
            const f = (progress - 0.2) / 0.3;
            skyTop = lerpColor(sunriseTop, dayTop, f);
            skyBottom = lerpColor(sunriseBottom, dayBottom, f);
            ambientLight = 0.7 + 0.3 * f;
        } else if (progress < 0.7) { // Sunset
            const f = (progress - 0.5) / 0.2;
            skyTop = lerpColor(dayTop, sunsetTop, f);
            skyBottom = lerpColor(dayBottom, sunsetBottom, f);
            ambientLight = 1.0 - 0.5 * f;
        } else { // Night
            const f = (progress - 0.7) / 0.3;
            skyTop = lerpColor(sunsetTop, nightTop, f);
            skyBottom = lerpColor(sunsetBottom, nightBottom, f);
            ambientLight = 0.5 - 0.2 * f;
        }
        ...
        if (progress < 0.75) {
            ...
            const sunX = w * (0.05 + 0.9 * sunProgress);
            const sunY = h * (0.6 - 0.45 * Math.sin(Math.PI * sunProgress));
            const sunRad = w * 0.12 * (1.0 + 0.08 * Math.sin(time * 0.0003)); // Slower sun pulse
            
            const sunGrad = ctx.createRadialGradient(sunX, sunY, 5, sunX, sunY, sunRad);
            const intensity = progress < 0.2 ? (progress / 0.2) : (progress > 0.5 ? (0.75 - progress) / 0.25 : 1.0);
            sunGrad.addColorStop(0, `rgba(255, 255, 220, ${0.85 * intensity})`);
            sunGrad.addColorStop(0.3, `rgba(255, 230, 150, ${0.4 * intensity})`);
            sunGrad.addColorStop(1, 'rgba(255, 255, 255, 0)');
            ctx.fillStyle = sunGrad;
            ctx.fillRect(0, 0, w, h);
        } else {
            // Moon is visible
            const moonProgress = (progress - 0.75) / 0.25;
            const moonX = w * (0.05 + 0.9 * moonProgress);
            const moonY = h * (0.6 - 0.45 * Math.sin(Math.PI * moonProgress));
            
            // Draw moon glow
            const moonGrad = ctx.createRadialGradient(moonX, moonY, 2, moonX, moonY, w * 0.06);
            moonGrad.addColorStop(0, 'rgba(230, 240, 255, 0.5)');
            moonGrad.addColorStop(1, 'rgba(230, 240, 255, 0)');
            ...
            ctx.beginPath();
            ctx.arc(moonX, moonY, 15, -Math.PI/3, Math.PI/3 * 2);
            ctx.quadraticCurveTo(moonX - 5, moonY, moonX, moonY - 13);
            ctx.closePath();
            ctx.fill();
        }
        ...
        const drawCloud = (cx, cy, scale) => {
            const cloudAlpha = progress > 0.7 ? 0.25 : 0.6;
        ...
        const cloudSpeed = 0.003; // Slower cloud movement
        const cloud1X = ((time * cloudSpeed + 100) % (w + 200)) - 100;
        drawCloud(cloud1X, h * 0.16, 1.25);
        const cloud2X = (((time * cloudSpeed * 0.7) + w * 0.55) % (w + 200)) - 100;
        drawCloud(cloud2X, h * 0.28, 0.85);
        
        // 3. Grass Hill
        const hillColor = lerpColor('#55a630', '#184710', progress > 0.7 ? (progress - 0.7)/0.3 : (progress < 0.2 ? (0.2 - progress)/0.2 : 0.0));
        ctx.fillStyle = hillColor;
        ctx.beginPath();
        const hillYOffset = Math.sin(time * 0.0001) * 4; // Slower hill sway
        ctx.ellipse(w * 0.5, h + 100 + hillYOffset, w * 0.75, h * 0.42, 0, 0, Math.PI * 2);
        ctx.fill();
        
        // 4. Growing and Swaying Trees
        trees.forEach(tData => {
            // Grow tree scale
            tData.currentScale = Math.min(tData.targetScale, tData.currentScale + tData.growthSpeed * 16.67);
            
            const sc = tData.currentScale;
            const tX = w * tData.x;
            const tY = h * 0.86 + hillYOffset;
            const sway = Math.sin(time * tData.swaySpeed) * tData.swayAmp * sc;
            
            // Draw Trunk
            ctx.fillStyle = tData.trunkColor;
            ctx.beginPath();
            ctx.moveTo(tX - 14 * sc, tY);
            ctx.quadraticCurveTo(tX - 8 * sc, tY - 80 * sc, tX - 25 * sc + sway * 0.5, tY - 170 * sc);
            ctx.lineTo(tX - 3 * sc + sway * 0.5, tY - 170 * sc);
            ctx.quadraticCurveTo(tX + 12 * sc, tY - 80 * sc, tX + 18 * sc, tY);
            ctx.closePath();
            ctx.fill();
            
            // Draw Foliage
            const fX = tX - 14 * sc + sway * 0.5;
            const fY = tY - 170 * sc;
            
            ctx.fillStyle = tData.leavesColor1;
            ctx.beginPath();
            ctx.arc(fX - 45 * sc + sway * 0.3, fY - 15 * sc, 50 * sc, 0, Math.PI * 2);
            ctx.arc(fX + 45 * sc + sway * 0.3, fY - 25 * sc, 55 * sc, 0, Math.PI * 2);
            ctx.arc(fX + sway * 0.3, fY - 60 * sc, 60 * sc, 0, Math.PI * 2);
            ctx.fill();
            
            // Draw Foliage layer 2
            ctx.fillStyle = tData.leavesColor2;
            ctx.beginPath();
            ctx.arc(fX - 25 * sc + sway * 0.4, fY + 10 * sc, 40 * sc, 0, Math.PI * 2);
            ctx.arc(fX + 35 * sc + sway * 0.4, fY - 10 * sc, 40 * sc, 0, Math.PI * 2);
            ctx.fill();
        });
        
        // 5. Playing/Running Children Silhouettes (running slower and less frequently)
        childSpawnTimer -= 16.67;
        if (childSpawnTimer <= 0 && childrenSilhouettes.length < 2) {
            // Spawn a new child
            const fromLeft = Math.random() < 0.5;
            childrenSilhouettes.push({
                x: fromLeft ? -40 : w + 40,
                targetX: fromLeft ? w + 60 : -60,
                dir: fromLeft ? 1 : -1,
                speed: 0.35 + Math.random() * 0.25, // Slower running speed
                scale: 0.5 + Math.random() * 0.3,
                phaseOffset: Math.random() * 100,
                jumpFreq: 0.02 + Math.random() * 0.015 // Slower jumping/skipping
            });
            childSpawnTimer = 25000 + Math.random() * 30000; // Less frequent spawns (25-55s)
        }
        
        // Update and draw children
        childrenSilhouettes = childrenSilhouettes.filter(c => {
            c.x += c.dir * c.speed;
            
            // Calculate child Y position on the grass hill
            const dx = c.x - (w * 0.5);
            const rx = w * 0.75;
            const ry = h * 0.42;
            let hillSurfaceY = h * 0.85;
            if (Math.abs(dx) < rx) {
                hillSurfaceY = (h + 100 + hillYOffset) - ry * Math.sqrt(1 - (dx * dx) / (rx * rx));
            }
            
            // Skip/bounce animation
            const skipY = Math.abs(Math.sin(time * c.jumpFreq + c.phaseOffset)) * 12 * c.scale;
            const cy = hillSurfaceY - skipY;
            
            // Draw silhouette
            ctx.fillStyle = progress > 0.75 ? 'rgba(5, 12, 25, 0.9)' : 'rgba(25, 45, 15, 0.8)';
            const r = 6 * c.scale;
            
            // Head
            ctx.beginPath();
            ctx.arc(c.x, cy - 22 * c.scale, r, 0, Math.PI * 2);
            ctx.fill();
            
            // Body
            ctx.beginPath();
            ctx.moveTo(c.x - 3 * c.scale, cy - 16 * c.scale);
            ctx.lineTo(c.x + 3 * c.scale, cy - 16 * c.scale);
            ctx.lineTo(c.x + 8 * c.scale, cy - 4 * c.scale);
            ctx.lineTo(c.x - 8 * c.scale, cy - 4 * c.scale);
            ctx.closePath();
            ctx.fill();
            
            // Legs (slower leg swing)
            const legSwing = Math.sin(time * 0.005 + c.phaseOffset) * 8 * c.scale;
            ctx.lineWidth = 2 * c.scale;
            ctx.strokeStyle = ctx.fillStyle;
            
            ctx.beginPath();
            ctx.moveTo(c.x - 3 * c.scale, cy - 4 * c.scale);
            ctx.lineTo(c.x - 4 * c.scale + legSwing, cy + 2 * c.scale);
            ctx.moveTo(c.x + 3 * c.scale, cy - 4 * c.scale);
            ctx.lineTo(c.x + 4 * c.scale - legSwing, cy + 2 * c.scale);
            ctx.stroke();
            
            // Arms (slower arm wave)
            const armWave = Math.sin(time * 0.006 + c.phaseOffset) * 6 * c.scale;
            ctx.beginPath();
            ctx.moveTo(c.x - 4 * c.scale, cy - 14 * c.scale);
            ctx.lineTo(c.x - 10 * c.scale, cy - 10 * c.scale + armWave);
            ctx.moveTo(c.x + 4 * c.scale, cy - 14 * c.scale);
            ctx.lineTo(c.x + 10 * c.scale, cy - 10 * c.scale - armWave);
            ctx.stroke();
            
            return (c.dir === 1 && c.x < w + 60) || (c.dir === -1 && c.x > -60);
        });
        
        // 6. Particles
        particles.forEach(p => {
            p.x += p.vx + Math.sin(time * p.wiggleSpeed + p.wiggleOffset) * 0.35;
            p.y += p.vy;
            if (p.x > w + 20) p.x = -20;
            if (p.y < -20) {
                p.y = h + 20;
            ...
            const pAlpha = p.alpha * (progress > 0.7 ? 1.0 : 0.4);
            const pColor = progress > 0.7 ? `rgba(230, 255, 140, ${pAlpha})` : `rgba(180, 240, 150, ${pAlpha})`;
...
function showSubtitle(text) {
    const subtitleDiv = document.getElementById('cutscene-subtitles');
    const subtitleSpan = document.getElementById('subtitle-text');
    if (subtitleDiv && subtitleSpan) {
        subtitleSpan.innerText = text;
        subtitleDiv.classList.remove('overlay-hidden');
    }
}
...
function hideSubtitle() {
    const subtitleDiv = document.getElementById('cutscene-subtitles');
    if (subtitleDiv) {
        subtitleDiv.classList.add('overlay-hidden');
    }
}

function triggerSpawnSamosbor() {
    state.spawnSamosborActive = true;
    ...
    // Hide ALL HUD elements by fading out the main interface
    ...
    if (scene && scene.fog) {
        scene.fog.color.setHex(0x800080); // Purple
        scene.fog.density = 0.15;
    }
    const canvas = document.getElementById('gameCanvas');
    if (canvas) {
        canvas.style.filter = "blur(8px)";
        canvas.style.transition = "filter 4s ease, opacity 4s ease";
    }
    
    // Subtitles sequence via overlay
    setTimeout(() => {
...
function triggerGameOver(reason) {
    ...
    switch (reason) {
        ...
        case "ending_2":
            ...
            title.innerText = t('go_ending2_title');
            title.style.color = "#ff3333";
            badgeText = t('go_badge_ending2');
            badgeClass = "badge-red";
            text = t('go_ending2_desc');
            if (audioCtx) {
                playScaryMusic();
            }
            break;
        ...
        case "ending_3":
            unlockAchievement("truth_dead");
            title.innerText = t('go_ending3_title');
            title.style.color = "#800080";
            badgeText = t('go_badge_ending3');
            badgeClass = "badge-red";
            text = t('go_ending3_desc');
            if (audioCtx) {
                playScaryMusic();
            }
            break;
...
function updateHUD() {
    ...
    if (btnLockRoom) {
        if (state.samosborSafe) {
            btnLockRoom.innerText = t('btn_unlock_room');
            btnLockRoom.classList.remove('glow-green');
            btnLockRoom.classList.add('glow-red');
        } else {
            btnLockRoom.innerText = t('btn_lock_room');
    ...
    if (btnMask) {
        btnMask.innerText = state.maskOn
            ? t('btn_mask_remove')
            : t('btn_mask');
    }
    
    const btnBandage = document.getElementById('btn-bandage');
    if (btnBandage) {
        btnBandage.innerText = t('btn_bandage', state.bandages || 0);
        if ((state.bandages || 0) > 0) {
            btnBandage.classList.remove('btn-disabled');
            btnBandage.disabled = false;
        } else {
            btnBandage.classList.add('btn-disabled');
            btnBandage.disabled = true;
        }
    }
...
function startGameLoop() {
    ...
    gameInterval = setInterval(() => {
        ...
        if ((isContaminated || isGasLeakHallway) && !state.maskOn) {
            ...
            logToConsole(isContaminated ? t("log_air_hazard") : t("log_gas_leak_suffocate"), "danger");
            playSoundDamage();
            if (state.health <= 0) {
        ...
        if (state.maskOn) {
            ...
            if (state.filter <= 0) {
                logToConsole(t("log_filter_clogged"), "danger");
                state.maskOn = false;
                updateHUD();
            }
        }
        ...
        if (state.samosborStatus === 'normal') {
            ...
            if (state.samosborCountdown <= 0) {
                ...
                logToConsole(t("log_samosbor_active_start"), "danger");
            ...
            if (state.spawnSamosborActive) {
                // Во время этой секретной концовки урон и стандартный таймер Самосбора не действуют, так как идет катсцена
            } else {
                ...
                if (state.location === 'hallway') {
                    if (!state.maskOn) {
                        state.health -= 15;
                        logToConsole(t("log_samosbor_toxic_lungs"), "danger");
                    } else {
                        state.health -= 2;
                    ...
                    if (!isSafe) {
                        ...
                        if (!state.maskOn) {
                            state.health -= 8;
                            logToConsole(t("log_gas_seeping_door"), "danger");
                        } else {
                            state.health -= 1;
                        }
                    } else {
                        // Заблокированная комната - безопасно
                        logToConsole(t("log_door_holding_rumble"), "sys");
                    }
                }
                ...
                if (state.samosborActiveDuration <= 0) {
                    ...
                    logToConsole(t("log_samosbor_end"), "sys");
                    stopSoundSiren();
...
function restartGame() {
    ...
    logToConsole(t("log_start_shift_floor"), "sys");
...
function initAudio() {
    ...
    try {
        ...
        logToConsole(t("log_sound_init"), "sys");
    } catch (e) {
...
function playSoundStep(isSprinting = false) {
    if (isSprinting) {
        playMP3(sndRun);
    } else {
        playMP3(sndSteps);
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.getElementById('btn-rebind-save').addEventListener('click', () => {
        saveKeyBindings();
        document.getElementById('settings-modal').classList.add('modal-hidden');
        logToConsole(t("log_save_controls"), "sys");
    });
    ...
    const btnBandage = document.getElementById('btn-bandage');
    if (btnBandage) btnBandage.addEventListener('click', useBandage);
    ...
    document.getElementById('btn-hack-cancel').addEventListener('click', () => {
        document.getElementById('hack-modal').classList.add('modal-hidden');
        hackActive = false;
        logToConsole(t("log_hack_cancelled"), "warn");
    });
    ...
    window.addEventListener('keydown', (e) => {
        ...
        const bandageKey = state.keyBindings['Bandage'] || 'KeyH';
            ...
            if (state.location === 'room' && !state.isSearching) {
                ...
                if (layout) {
                    ...
                    if (playerPos.distanceTo(cabPos) < 3.5 || playerPos.distanceTo(tablePos) < 3.0) {
                        ...
                        logToConsole(t("log_approach_furniture"), "warn");
                    }
                }
            }
        }
    ...
    window.addEventListener('mousedown', (e) => {
        if (window.devConsoleOpen || isGamePaused || hackActive || state.cutsceneActive) return;
    ...
    window.addEventListener('click', (e) => {
        ...
        if (notesModal && !notesModal.classList.contains('modal-hidden')) return;
    ...
    document.addEventListener('mousemove', (e) => {
        ...
        if (pointerLocked && !isGamePaused) {
            playerYaw -= e.movementX * sensitivity * mouseSensMultiplier;
            playerPitch -= e.movementY * sensitivity * mouseSensMultiplier;
    ...
    canvasHolder.addEventListener('mousemove', (e) => {
        ...
        if (isDragging && !pointerLocked && !isGamePaused) {
            playerYaw -= e.movementX * sensitivity * mouseSensMultiplier * 1.5;
            playerPitch -= e.movementY * sensitivity * mouseSensMultiplier * 1.5;
            playerPitch = Math.max(-Math.PI / 2.5, Math.min(Math.PI / 2.5, playerPitch));
        }
    });
...
document.getElementById('btn-descend').addEventListener('click', () => {
    ...
    if (state.notesCount === 10 && allNotesRead && (state.samosborStatus === 'warning' || state.samosborStatus === 'active')) {
        disableAllControls(true);
        logToConsole(t("log_ending_freeze_wait"), "danger");
...
(function() {
    ...
    function executeDevCommand(raw) {
        ...
        if (russianAliases[cmd]) {
            const alias = russianAliases[cmd].split(/\s+/);
            cmd = alias[0];
            if (alias[1] && !parts[1]) parts[1] = alias[1];
        }
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    // Sensitivity (Event Delegation for reliability)
