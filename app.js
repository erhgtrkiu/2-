...
const sndBgMusic = new Audio('sounds/The_Iron_Threshold.mp3');
sndBgMusic.loop = true;
sndBgMusic.volume = 0.4;
...
function updateInventoryUI() {
    ...
    if (bagBatteries) {
        bagBatteries.innerText = `${t("battery_qty", state.batteries)}`;
    }
}
...
function stopAtmosphere() {
    ...
    if (!sndBgMusic.paused) sndBgMusic.pause();
...
function restartGame() {
    ...
    if (audioCtx) {
        startAtmosphere();
        if (sndBgMusic.paused) sndBgMusic.play().catch(e => console.warn(e));
        startHeartbeatLoop();
    }
    state = {
        floor: START_FLOOR,
...
function initAudio() {
    ...
    try {
        ...
        if (ACHIEVEMENTS.awakened && ACHIEVEMENTS.awakened.unlocked && inMenu) {
            ...
            startAtmosphere();
            startHeartbeatLoop();
            if (sndBgMusic.paused) {
                sndBgMusic.play().catch(e => console.warn(e));
            }
        }
